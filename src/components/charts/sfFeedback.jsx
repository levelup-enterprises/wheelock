...
const SFFeedback = ({ sf, active = false }) => {
  const { desktop, mobile, previous, trend } = sf;
  const [showInfo, toggleInfo] = useState(false);
  const { user } = useContext(UserContext);

  const doAction = (e) => {
    // console.log(e);
    if (user.workspace === "sf") {
      let query = {};
      if (e.id === "Helpful") {
        query = {
          helpful: ["equals", "Yes"],
      ...
      {showInfo ? (
        <div className="info">
          ...
          <Button
            ...
            onClick={() => active.updateWidget("sfFeedback")}
          />
        </div>
      ) : (
        <>
          <div className="top-container">
            <div className="top-wrapper">
              ...
              {previous ? (
                setPrevious(sf.total, previous.total)
              ) : (
                <>
                  <span>- </span>
                  <span className="tiny">{trend}</span>
          ...
          <div className="structure-wrapper">
            <div className="chart-wrapper" style={{ minHeight: "150px" }}>
              <ResponsivePie
                ...
                innerRadius={0.7}
                padAngle={sf.percent === 100 || sf.percent === 0 ? 0 : 4}
