...
const QPFeedback = ({ qp, active = false }) => {
  const { desktop, mobile, previous, trend } = qp;
  const [showInfo, toggleInfo] = useState(false);
  const { user } = useContext(UserContext);

  const doAction = (e) => {
    console.log(e);
    if (user.workspace === "qp") {
      let query = {};
      if (e.id === "Helpful") {
        query = {
          helpful: ["equals", "Yes"],
  ...
  const data = [
    ...
    },
    {
      id: "Not Helpful",
      label: "Not Helpful",
      value: qp.total - qp.helpful,
    },
  ];

  return (
    <div
      className={"card oamTotal" + (active && active.disabled ? " active" : "")}
      title={
        active
          ? active.disabled
    ...
    >
      <h5 className="title">
        Quickpay Feedback<span className="comparison"></span>
      </h5>
      {showInfo ? (
        <div className="info">
          ...
          <Button
            ...
            onClick={() => active.updateWidget("qpFeedback")}
          />
        </div>
      ) : (
        <>
          <div className="top-container">
            <div className="top-wrapper">
              ...
              {previous ? (
                setPrevious(qp.total, previous.total)
              ) : (
                <>
                  <span>- </span>
                  <span className="tiny">{trend}</span>
          ...
          <div className="structure-wrapper">
            <div className="chart-wrapper" style={{ minHeight: "150px" }}>
              <ResponsivePie
                ...
                innerRadius={0.7}
                padAngle={qp.percent === 100 || qp.percent === 0 ? 0 : 4}
